...
$path       = 'C:\LogRhythm\symantec-cloud'

##### Create our scheduled task #####
$executable = "py -3 $path\sepc.py"
$taskName   = 'Download Symantec Cloud logs for LogRhythm'

$action   = New-ScheduledTaskAction -Execute py -Argument "-3 $path\sepc.py"
...
$robocopy_args = "/e"
if (Test-Path ("$path\sepc.conf")) {
  $robocopy_args += " /xf sepc.conf"
}
...
##### Fire up wordpad to edit sepc.conf #####
Start-Process -FilePath "$($env:Programfiles)\Windows NT\Accessories\wordpad.exe" -ArgumentList "$path\sepc.conf"